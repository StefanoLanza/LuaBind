...
template <class Type>
class Wrapper<std::shared_ptr<Type>> {
public:
	static int Match(lua_State* ls, int idx) {
		const int type = lua_type(ls, idx);
		return (type == LUA_TLIGHTUSERDATA || type == LUA_TUSERDATA || type == LUA_TTABLE || type == LUA_TNIL);
	}
	static int Push(lua_State* ls, const std::shared_ptr<Type>& ptr) {
		return LUA::Push(ls, ptr.get());
	};
#if 0
...
#endif
	static std::shared_ptr<Type> Get(lua_State* /*ls*/, int /*idx*/) {
		return {};
	}
